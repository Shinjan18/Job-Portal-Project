...
const wrappedUpload = (req, res, next) => {
  upload.single('resume')(req, res, (err) => {
    if (err) {
      ...
      return res.status(500).json({ message: 'Multer Error', error: err.toString(), stack: err.stack });
    }
    next();
  });
};

// Alias for /mine/list to match requirements - MOVED UP
...
router.post('/:id/apply', auth, wrappedUpload, async (req, res) => {
  try {
    const job = await Job.findById(req.params.id);
    if (!job) return res.status(404).json({ message: 'Job not found' });
    ...
    // Get user details (already in req.user due to auth middleware)
    const user = req.user;
    ...
    const application = await Application.create({
      ...
      pdfUrl: pdfUrl,
      resumeUrl: resumeUrl,
      accountEmail: user.email // Store source of truth email
    });
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Soft-Auth: Check if user is logged in via token
      let userId = null;
      ...
      if (token) {
        try {
          ...
          console.log('SoftAuth: Token verified, payload:', payload);
          const user = await User.findById(payload.id);
          if (user) {
            console.log('SoftAuth: User found:', user.email);
            userId = user._id;
            accountEmail = user.email;
          } else {
            console.log('SoftAuth: User not found for ID:', payload.id);
          }
        } catch (err) {
          console.error('SoftAuth: Token verification failed:', err.message);
          // Ignore invalid tokens for quick apply (treat as guest)
        }
      } else {
        console.log('SoftAuth: No token found. Header:', header);
      }
